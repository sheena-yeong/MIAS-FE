...
import { 
  ...
  IoTimeSharp, 
  IoSettingsSharp 
} from 'react-icons/io5';import { LuLayoutDashboard } from "react-icons/lu";
...
function SideBar() {
  ...
  return (
    <>
    <img src={miasLogo} className="p-3 mt-3 mb-5"/>
      ...
      {navItems.map(item => (
        <button
          ...
          className={`flex items-center gap-2 py-3 text-lg w-full text-left px-3 rounded-md transition-colors
            ${active === item.name ? 'bg-[#ff5c5c] text-black' : 'hover:bg-[#4a4949]'}`}
        >
